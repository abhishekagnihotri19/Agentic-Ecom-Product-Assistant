...
class PromptTemplate:
    ...
    def format (self, **kwargs)->str:
        """Valdation of Missing Placeholders"""
        missing= [f for f in self.required_placeholders() if f not in kwargs]
        if missing:
            raise ValueError (f"Required Placeholder is missing: {missing}")
        return self.template.format(**kwargs)
    
    def required_placeholder(self):
        return [field_name for _,field_name,_,_ in string.Formatter.parse(self.template) if field_name] 
    
PromptRegistry : Dict[PromptType, PromptTemplate]= {
